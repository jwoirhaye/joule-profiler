...
NVML (distributed as `libnvidia-ml.so` on Linux or `nvml.dll` on Windows) communicates directly with the NVIDIA GPU driver to query device state.

![NVML architecture](../../figures/nvml_architecture.png)
